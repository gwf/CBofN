...
double crate = 0.25, mrate = 0.01, target = 0.5, var = 1;
...
OPTION options[] = {
  { "-target", OPT_DOUBLE,  &target, "Target value for function." },
  { "-var",    OPT_DOUBLE,  &var,    "Variance of bump." },
...
/* 1-D bump function centered at target with variance var. */

double bump(double x)
{
  return(exp(-(x - target) * (x - target) / var ));
}
